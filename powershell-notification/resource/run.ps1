...
if ($provisioningState -match "Succeeded" ) {
    ...
    # fetching toeken for managed identity
    $listTokenUri = 'https://management.azure.com' + $applicationId + '/listTokens?api-version=2018-09-01-preview'
    $body = @{
        "authorizationAudience"="https://management.azure.com/"
    } |ConvertTo-Json
    $mIdTokenResponse = Invoke-RestMethod -Uri $listTokenUri -ContentType "application/json" -Method POST -Body $body -Headers @{"Authorization" = "Bearer $accessToken"; "client_id" = $managedIdentity }
    Write-Host $mIdTokenResponse
    Write-Host $mIdTokenResponse.access_token
